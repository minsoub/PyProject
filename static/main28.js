...
var localhost='cat.ks.ac.kr/RES4';
		...
		var is_init_dz=true;
		var json_file_name, excel_file_name;
...
//		Dropzone.options.demoUpload = {
		$('#demo_upload').dropzone({
			maxFilesize: 100, // MB
			maxFiles : 2,
            dictDefaultMessage: "Drag your excel file",
			parallelUploads: 2,
//			acceptedMimeTypes: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/excel,application/vnd.ms-excel',
			acceptedFiles: ".xls, .xlsx, .xlsm, .json",
			uploadMultiple: false,
            init: function() {
				  /*this.on("success", function(file, responseText) {
					file.previewTemplate.setAttribute('id',responseText[0].id);
				  });*/
				var mockFile1 = { name: "tr5.xlsm", size: 12345};       			  
				var mockFile2 = { name: "model1.2.json", size: 12345};       			  
				this.options.addedfile.call(this, mockFile2);
				this.files.push(this, mockFile2);
				this.emit("complete", mockFile2);
				json_file_name=mockFile2.name
				this.options.addedfile.call(this, mockFile1);
				this.files.push(this, mockFile1);
				this.emit("complete", mockFile1);
				excel_file_name=mockFile1.name
...
//					dzClosure.processQueue();					
			...
			accept: function(file, done) {
					...
					if (is_init_dz || this.files.length==3){
						this.removeFile(file);
						return;
					}
					if (ext && ext.match(/\.xls|\.xlsx|\.xlsm|\.json/)) {
						if (ext && ext.match(/\.xls|\.xlsx|\.xlsm/)) {
							excel_file_name=file.name
						}else if (ext && ext.match(/\.json/)) {
					...
					file.rejectDimensions = function() { done("You can't upload files of this type."); };
			},          
			error: function(file, message) {
				var ext = file.name.substr(file.name.lastIndexOf('.'));
				if (ext && ext.match(/\.xls|\.xlsx|\.xlsm/)) {
					alert('excel file already exist!');
				}else if (ext && ext.match(/\.json/)) {
					alert('json file already exist!');
				}else{
					alert('excel or json file !');					
				}
				this.removeFile(file);
			},
...
//	var urlname='http://'+localhost+'/php_process/read_json_file.php?jsonFileName=model/'+json_file_name;
	var urlname='http://'+localhost+'/php_process/read_json_file.php?jsonFileName=model/'+json_file_name+'&excelFileName=model/'+excel_file_name;
	read_json(urlname);	  // read json file
	json=from_ajax_data;
	
	var value = $('progress:first').prop('value'); 
	$('#progressbar').val(0);
	$('progress:first').val(0); 
    $('#fyear').val(json.general.fyear);
    $('#lyear').val(json.general.lyear);
    $('#tsSheetName').html('<label><font color="blue">'+excel_file_name+'!'+'</font><font color="red">'+json.general.tsSheetName+'</font></label>');
    $('#solSheetName').html('<label><font color="blue">'+json.general.solExcelName+'!'+'</font><font color="red">'+json.general.solSheetName+'</font></label>');
	json.excelFileName=excel_file_name;
...
$("#read_model").on("mousedown", function () {
    $('#read_model').html('now processing...');
}).on('mouseup', longFunc);
...
function longFunc() {
	...
	make_contextmenu();
	make_tech_var_menu();
    $('#read_model').html('Read json/excel data');
}
...
function json_editor_update(json){
...
function general_change(btn){
    ...
    if (val==''){
		...
		if(cur_name=="fyear" || cur_name=="lyear"){
			json.general[cur_name]=parseInt(val);
		}
	}
	//json_editor_update(json);
}
...
var temp_json;
function remove_ts_sol_ids_etc_in_json(){
	temp_json=JSON.parse(JSON.stringify(json));  // make copy_by_value !!!!
	for (i=0;i<temp_json.energyLevels.length;i++){
		var el=temp_json.energyLevels[i];
		if (el.hasOwnProperty('l_id')) delete el.l_id;
		for (j=0;j<el.energyForms.length;j++){
			var ef=el.energyForms[j];
			if (ef.hasOwnProperty('f_id')) delete ef.f_id;
			if (ef.hasOwnProperty('P')) delete ef.P;
			if (ef.hasOwnProperty('C')) delete ef.C;
			if (ef.hasOwnProperty('demand')){
				if (ef.demand.hasOwnProperty('ts')) 
					delete ef.demand.ts;
			}
		}
	}
	for (i=0;i<temp_json.techLevels.length;i++){
		var tl=temp_json.techLevels[i];
		if (tl.hasOwnProperty('tl_id')) delete tl.tl_id;
		for (j=0;j<tl.techs.length;j++){
			var tech=tl.techs[j];
			if (tech.hasOwnProperty('t_id')) delete tech.t_id;
			if (tech.hasOwnProperty('sol_y')) delete tech.sol_y;
			if (tech.capacity.inv_cost.hasOwnProperty('ts')) delete tech.capacity.inv_cost.ts;
			if (tech.capacity.fixed_cost.hasOwnProperty('ts')) delete tech.capacity.fixed_cost.ts;
			if (tech.capacity.hasOwnProperty('H')) delete tech.capacity.H;
			for (k=0;k<tech.activities.length;k++){
				var act=tech.activities[k];
				if (act.hasOwnProperty('sol_x')) delete act.sol_x;
				if (act.var_cost.hasOwnProperty('ts')) delete act.var_cost.ts;
				for (ii=0;ii<act.inputs.length;ii++){
					var inp=act.inputs[ii];
					//if (inp.hasOwnProperty('f_id')) delete inp.f_id;
					if (!inp.hasOwnProperty('is_main'))
						if (inp.value.hasOwnProperty('ts')) delete inp.value.ts;
				}
				for (ii=0;ii<act.outputs.length;ii++){
					var outp=act.outputs[ii];
					//if (outp.hasOwnProperty('f_id')) delete outp.f_id;
					if (outp.value.hasOwnProperty('ts')) delete outp.value.ts;
				}
			}				
		}
	}
	//if (temp_json.hasOwnProperty('solutions')) 		delete temp_json.solutions;
	
	for (i=0;i<temp_json.constraints.type1.length;i++){
		if (temp_json.constraints.type1[i].hasOwnProperty('ts')) 
			delete temp_json.constraints.type1[i].ts;
		if (temp_json.constraints.type1[i].hasOwnProperty('c1_id')) 
			delete temp_json.constraints.type1[i].c1_id;
	}
	for (i=0;i<temp_json.constraints.type2.length;i++){
		if (temp_json.constraints.type2[i].hasOwnProperty('ts')) 
			delete temp_json.constraints.type2[i].ts;
		if (temp_json.constraints.type2[i].hasOwnProperty('c2_id')) 
			delete temp_json.constraints.type2[i].c2_id;
	}
	for (i=0;i<temp_json.constraints.type3.length;i++){
		if (temp_json.constraints.type3[i].hasOwnProperty('ts')) 
			delete temp_json.constraints.type3[i].ts;
		if (temp_json.constraints.type3[i].hasOwnProperty('c3_id')) 
			delete temp_json.constraints.type3[i].c3_id;
	}
	for (i=0;i<temp_json.constraints.short_type1.length;i++){
		var st=temp_json.constraints.short_type1[i];
		for (j=0;j<st.techs.length;j++){
			var tech=st.techs[j];
			if (tech.lower.hasOwnProperty('ts')) delete tech.lower.ts;
			if (tech.upper.hasOwnProperty('ts')) delete tech.upper.ts;			
			if (tech.equal.hasOwnProperty('years')){
				if (tech.equal.years.hasOwnProperty('ts')) delete tech.equal.years.ts;
			}
			if (tech.equal.RHS.hasOwnProperty('ts')) delete tech.equal.RHS.ts;
		}
	}
	
	for (i=0;i<temp_json.solutions.length;i++){
		for (j=0;j<temp_json.solutions[i].techs.length;j++){
			if (temp_json.solutions[i].techs[j].hasOwnProperty('sol')) 
				delete temp_json.solutions[i].techs[j].sol;						
		}
	}
		
	if (temp_json.hasOwnProperty('excelFileName')) 
		delete temp_json.excelFileName;
}
function save_json(btn){
	remove_ts_sol_ids_etc_in_json();
    $.ajax
    ({
        type: "POST",
       // dataType : 'text',  //dataType : 'json' ==> 200 OK, SyntaxError  
		cache:false,
        async: false,
        url: 'http://'+localhost+'/php_process/general_save_json.php',
        data: { data: JSON.stringify(temp_json), fileName: json_file_name},
//        data: { data: JSON.stringify(json_obj), fileName: 'model1.json' },
		dataType: 'json',
        success: function(data) {		 
			json=data;
			json_editor_update(temp_json);
			alert( "File saved : model/" + json_file_name );
        },
        error: 	function(request,status,error){
        alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error)}
	}).done(function( msg ) {
...
function populate_json(){	
	//redraw();
	populate_general();					//$('progress:first').val(10);
	update_all_ts('energyLevels');		//$('progress:first').val(20);
	populate_energy_form();				//$('progress:first').val(30);
	update_all_ts('techLevels');			//$('progress:first').val(40);
	populate_technology();				//$('progress:first').val(50);
	populate_variable();					//$('progress:first').val(60);
	populate_contraint_short_type1();		//$('progress:first').val(70);
	populate_contraint_type1();			//$('progress:first').val(80);
	populate_contraint_type2();			//$('progress:first').val(90);
	populate_contraint_type3();			//$('progress:first').val(100);
}
...
$("#fyear").change(function () {
    json.general.fyear = $('#fyear').val();
});
$("#lyear").change(function () {
    json.general.fyear = $('#lyear').val();
});
...
function getExcelRange(excelRange){
	if (isNaN(excelRange)){
		...
		var url_loc='http://'+localhost+'/php_process/get_ts.php?'
					+'&excelFileName='+excel_file_name+'&sheetName='+json.general.tsSheetName+'&newRange='+newRange;
		read_json(url_loc);
...
$(document).on('blur', '#tree_for_el_ef [contenteditable],#tree_for_tech [contenteditable],'
	...
	if($(this).closest('li').hasClass('var')){
		var is_ts=$(this).closest('li.var').find('input[name="is_ts_var"]').is(':checked');
		var years=json.general.lyear-json.general.fyear+1;
...
//	menu2['sep2']='---------';		
//	menu2['fold_deselect']={name: "deselect"};
}
...
function update_all_ts(target){
   ...
   //     url: url_loc,
        ...
        data: { data: JSON.stringify(json), excelFileName:excel_file_name, sheetName:json.general.tsSheetName,target:target},
//        data: { data: JSON.stringify(json_obj), fileName: 'model1.json' },
		dataType: 'json',
        success: function(data) {		 
		  json=data;
...
function add_io_act_tech_tl(btn){	
	...
	if ($(cur).hasClass('mainInputLi') || $(cur).hasClass('otherInputLi')
		...
		$(cur).next().find('.otherInputLi').remove();
		$(cur).next().find('.otherOutputLi').remove();		
		if ($(cur).closest('li.techLevelLi').find('.techUl').find('.techLi').length==2){
			$(cur).closest('li.techLevelLi').find('.techUl').find('.techLi').each(function(){
				$(this).find('.ef_button').removeAttr('disabled');
			});
		}
		...
		$(cur).next().find('.otherInputLi').remove();
		$(cur).next().find('.otherOutputLi').remove();		
		if ($('#tree_for_tech').find('.techLevelLi').length==2){
			$('#tree_for_tech').find('.techLevelLi').each(function(){
				$(this).find('.el_button').removeAttr('disabled');
			});
		}
		...
		for (var j=0;j<cur.techs.length;j++){
			...
			if (cur_tech.hasOwnProperty('is_ts') && !cur_tech.is_ts){
				...
				row+='<tr><td align="left" class="techVarName" style="border-right:1px solid black;">'+cur_var.name+'</td>'
					...
					+'<td><span class="excelRange" contenteditable>'+upper+'</span></td>'
					+'<td style="border-right:1px solid black;"><input name="addToLP_ub" class="ubBox" style="margin-left:1px;" type="checkbox"></td>'
					+'<td><span class="excelRange" contenteditable>'+cur_var.equal.years.excelRange+'</span></td>'
					+'<td><span class="excelRange" contenteditable>'+cur_var.equal.RHS.excelRange+'</span></td>'
					+'<td><input name="addToLP_eq" class="eqBox" style="margin-left:1px;" type="checkbox"></td>'
				+'</tr>';
			}
...
function update_short_type1(){	
	...
	$('#short_type1_constaintUl').find('li.constraint').each(function(){
		...
		$(table).find('tr:gt(1)').each(function(){
			...
			if ($(this).find('td:eq(5)').find('.excelRange').length){
...
function generate_lp_treeviewer(btn){
	...
	var lpObjUl = $('#lpObjUl');
	lp='';
	json.general.lp_name=json_file_name+'.lp';
	
	lp+='\\Problem name: ' + json_file_name + '\nMinimize';
...
//Bounds End
    ...
    //    url: url_loc,
        ...
        data: { data: lp, fileName: json.general.lp_name},
//        data: { data: JSON.stringify(json_obj), fileName: 'model1.json' },
		dataType: 'text',
        success: function(data) {		 
		...
		//  json=data;
		 // json=JSON.parse(data);
		 alert('lp saved in :' +'RES4/lp/'+ json.general.lp_name);
        },
        error: 	function(request,status,error){
        alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error)}
	}).done(function( msg ) {
...
function save_sol_to_excel_ajax(){
	//json.general.solExcelName='sol.xlsx'
//	json.general.solExcelName=excel_file_name;"sol.xlsx"
    $.ajax
    ({
        type: "POST",
 		cache:false,
		...
		dataType: 'json',
        success: function(data) {		 
//			console.log('excel file saved in lp/' + json.general.solExcelName + '!' + json.general.solSheetName);
			json=data;
			alert('solution saved in model/' + json.general.solExcelName + '!' + json.general.solSheetName);
        },
        error: 	function(request,status,error){
			alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error)
		}
