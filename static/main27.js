...
//			acceptedMimeTypes: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/excel,application/vnd.ms-excel',
            ...
            init: function() {
				...
				var mockFile2 = { name: "model1.1.json", size: 12345};       			  
...
// make energy form table	
	...
	for (var i = 0; i < json.energyLevels.length; i++) {
		...
		for (var j = 0; j <el.energyForms.length; j++){
			var ef=json.energyForms[el.energyForms[j]-1];
			//ef.f_id=cur_f_id++;
...
$(document).on('click', '#tree_for_el_ef .hasLoadRegion, #tree_for_el_ef .eq', function () {
	var f_id=parseInt($(this).closest('.ef').find('.id').html())-1;
	
	var col_id=$(this).attr('class');

	if (col_id=='hasLoadRegion' || col_id=='eq'){
		var cur_state=json.energyForms[f_id][col_id];
		if (cur_state){
			$(this).prop('checked', false);
			json.energyForms[f_id][col_id]=false;
		}else{
			$(this).prop('checked', true);
			json.energyForms[f_id][col_id]=true;
		}		
	}
	json_editor_update();
...
function getExcelRange(excelRange){
	if (isNaN(excelRange)){
		...
		var url_loc='http://'+localhost+'/php_process/get_ts.php?'
					+'&excelFileName='+excel_file_name+'&sheetName='+json.general.sheetName+'&newRange='+newRange;
		read_json(url_loc);
		...
		if ($(this).closest('li').hasClass('ef') || $(this).closest('li').hasClass('demand')){
			...
			if (dataType=="demandRange"){
				if (isNaN($(this).text())){
					...
					json.energyForms[f_id].demand['excelRange']=$(this).text();
					json.energyForms[f_id].demand['ts']=ts.toString();			
				}else {
					json.energyForms[arr_f_id].demand=$(this).text();
					var cur_td=$(this).closest('td').next().next();
				...
				json.energyForms[f_id][dataType]=$(this).text();						
				changed_el_ef("change_ef_name",arr_l_id, f_id);
				make_contextmenu();
			}				
			//json_editor_update();
			return;
...
function add_ef_after(btn) {
	...
	var f_id=parseInt($(btn).closest('.ef').find('.id').html())-1;
	var PrevNoOfenergyForms=json.energyLevels[arr_l_id].energyForms.length;
	json.energyForms.splice(f_id,0,json.energyForms[f_id]);
	...
	for (var i=arr_l_id;i<json.energyLevels.length;i++){
		...
		if (i==arr_l_id){
			...
			for (var j = f_id; j <cur_el.energyForms.length; j++){
				cur_ef_li=$(cur_ef_li).next();
				var cur_f_id=parseInt($(cur_ef_li).find('.id').html())
//				cur_el.energyForms[j].f_id++;
				$(cur_ef_li).find('.id').html(cur_f_id+1);
			}			
		}else{
...
//				cur_el.energyForms[j].f_id++;
	...
	changed_el_ef("add_ef",arr_l_id,f_id);
	make_contextmenu();		
	//json_editor_update();
...
function changed_el_ef(menu,arr_l_id, arr_f_id){
	...
	switch(menu){
		case "change_el_name" : 
			...
			break;
		case "change_ef_name" : 
			var new_ef_name=json.energyForms[arr_f_id].name;
			$("#tree_for_tech .context-menu-one").each(function(){
				if ($(this).text()){
					var matches = $(this).text().split('(');
					var temp=matches[1].split(')');
			...
			break;
		case "add_ef" : 
			//var f_id=json.energyForms[arr_f_id].f_id;
		...
		case "add_el" :
			var l_id=arr_l_id;
			var f_id=json.energyLevels[arr_l_id].f_id-1;
			$("#tree_for_tech .context-menu-one").each(function(){
				var regExp = /\(([^)]+)\)/g;
				if ($(this).text()){
					var temp=$(this).text().split('/');
...
function update_all_ts(target){
   ...
   //     url: url_loc,
        ...
        data: { data: JSON.stringify(json), excelFileName:excel_file_name, sheetName:json.general.sheetName,target:target},
//        data: { data: JSON.stringify(json_obj), fileName: 'model1.json' },
		dataType: 'json',
        success: function(data) {		 
		  json=data;
